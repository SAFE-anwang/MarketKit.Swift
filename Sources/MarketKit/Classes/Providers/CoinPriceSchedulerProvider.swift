...
extension CoinPriceSchedulerProvider: ISchedulerProvider {
    ...
    func sync() async throws {
        ...
        let src20CoinPrices = src20Prices.map { CoinPrice(coinUid: TokenQuery(blockchainType: .safe4, tokenType: .eip20(address: $0.address)).customCoinUid, currencyCode: "USD", value: Decimal(string: $0.price) ?? 0 , diff24h: Decimal(string: $0.change), diff1d: Decimal(string: $0.change), timestamp: Date().timeIntervalSince1970) }
