...
extension CoinPriceSchedulerProvider: ISchedulerProvider {
    ...
    func sync() async throws {
        ...
        let src20CoinPrices = src20Prices.map { CoinPrice(coinUid: TokenQuery(blockchainType: .safe4, tokenType: .eip20(address: $0.address)).customCoinUid, currencyCode: "USD", value: Decimal(string: $0.price) ?? 0 , diff24h: nil, diff1d: nil, timestamp: Date().timeIntervalSince1970) }
